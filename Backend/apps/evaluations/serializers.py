...
class TestAttemptSerializer(serializers.ModelSerializer):
    class Meta:
        model = TestAttempt
        fields = ('id', 'test', 'student', 'status', 'client_session_id', 'started_at', 'submitted_at')
        read_only_fields = ('id', 'student', 'status', 'client_session_id', 'started_at', 'submitted_at')
...
class StartTestSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        if attempt:
            if attempt.client_session_id:
                if not client_session_id:
                    raise serializers.ValidationError({'client_session_id': 'Active session in progress.'})
                ...
                attempt.client_session_id = client_session_id or uuid.uuid4().hex
        ...
        return TestAttempt.objects.create(
            ...
            client_session_id=client_session_id or uuid.uuid4().hex,
        )
...
class SaveResponseSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if attempt.client_session_id:
            if not client_session_id:
                raise serializers.ValidationError({'client_session_id': 'client_session_id is required.'})
            if attempt.client_session_id != client_session_id:
